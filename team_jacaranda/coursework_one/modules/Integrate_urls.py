...
# List of companies to add - each company is a dictionary containing the required information
COMPANIES_TO_ADD = [
    {
        "security": "Apple Inc.",  # Required
...
def modify_database(DB_PATH):
    """adding/deleting companies from"""
    conn = None
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        
        # Confirm if the table exsits
        cursor.execute("""
        ...
        if not cursor.fetchone():
            ...
            # Create table
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS equity_static (
        ...
        # delete given companies
        for company in COMPANIES_TO_DELETE:
            logger.info(f"Deleting: {company}")
            cursor.execute("DELETE FROM equity_static WHERE security = ?", (company,))
        
        # add given companies
        for company in COMPANIES_TO_ADD:
            logger.info(f"Adding: {company['security']}")
            # Check if it already exists
            cursor.execute("SELECT 1 FROM equity_static WHERE security = ?", (company['security'],))
            if cursor.fetchone():
                logger.info(f" {company['security']} exists,updating information")
                cursor.execute("""
                    UPDATE equity_static 
                    SET gics_sector = ?, gics_industry = ?, country = ?, region = ?, website = ?
                    WHERE security = ?
                """, (
                    company.get('gics_sector', ''),
                    company.get('gics_industry', ''),
                    company.get('country', ''),
                    company.get('region', ''),
                    company.get('website', ''),
                    company['security']
                ...
                logger.info(f"New company: {company['security']}")
                cursor.execute("""
                    INSERT INTO equity_static (security, gics_sector, gics_industry, country, region, website)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    company['security'],
                    company.get('gics_sector', ''),
        ...
        conn.commit()
        logger.info(f"Database modification completed: add {len(COMPANIES_TO_ADD)} companies, delete {len(COMPANIES_TO_DELETE)} companies")
        
    except Exception as e:
        logger.error(f"Database modification failed： {str(e)}")
        if conn:
            conn.rollback()
    finally:
        if conn:
